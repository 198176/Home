    ...
    public void onStart() {
        ...
        try {
            cursor = db.rawQuery("SELECT PAYMENT.TITLE, PAYMENT.VALUE, strftime('%Y-%m-%d', " +
                    ...
                    "CATEGORY WHERE PAYMENT.CATEGORY_ID=CATEGORY._id AND " + MainActivity.getPersonId() + " MONTH = '" + MainActivity.getSpinnerDate() +
                    "' AND DATE/1000 <= cast(strftime('%s', 'now') as integer) ORDER BY PAYMENT.DATE DESC, PAYMENT._id DESC", null);
        } catch (SQLiteException w) {
            Toast.makeText(getActivity(), R.string.database_error, Toast.LENGTH_SHORT).show();
        }
