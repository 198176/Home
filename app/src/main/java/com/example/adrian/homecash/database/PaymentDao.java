...
public interface PaymentDao {
    ...
    Cursor getAllPaymentsByDateAndUser(int dayBilling, String date, int payingId);
    ...
    Cursor getAllParticipantPaymentByUser(int dayBilling, String date, int personId);
    ...
    Cursor getPositiveSummaryPaymentsCategoriesByUser(int dayBilling, String date, int personId);

    @Query("SELECT name, color, icon, Categories.id, SUM(user_value) value FROM Payments, Categories, Participants WHERE " +
            "Participants.user_id = :personId AND Participants.payment_id = Payments.id AND Payments.category_id = Categories.id AND " +
            DATE_BY_BILLING + " = :date AND DATE/1000 <= cast(strftime('%s', 'now') as integer) AND value < 0 GROUP BY Categories.id")
    Cursor getNegativeSummaryPaymentsCategoriesByUser(int dayBilling, String date, int personId);
    ...
    double getMonthlyBalancePaymentsByUser(int dayBilling, int userId, String date);
    ...
    @Query("SELECT SUM(CASE WHEN category_id = 0 AND paying_id = 1 THEN value ELSE user_value END) FROM " +
            "Participants, Payments WHERE (paying_id = 1 OR (category_id = 0 AND user_id = 1)) AND Payments.id = payment_id")
    double getTotalBalancePayments();
